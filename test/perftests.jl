...
# 140dbf60-429f-11ea-3086-c7bbf0ba2b27
...
module perftests
using IntegerSequences, DataStructures, Dates, InteractiveUtils
...
@time T055883(100)
# +++ EulerTransforms.jl +++
println("\nTEST: (for n ∈ 0:150 V107895(n) end)")
...
println("\nTEST: (for n ∈ -100:100, k ∈ -100:100")
@time (for n ∈ -100:100, k ∈ -100:100
Binomial(n, k)
end)
println("\nTEST: (for k ∈ -10000:10000")
@time (for k ∈ -10000:10000
Binomial(-5, k)
end)
# +++ GeneralizedFibonacci.jl +++
println("\nTEST: (for n ∈ 0:1000 Multinacci(3, n) end)")
@time (for n ∈ 0:1000 Multinacci(3, n) end)
# +++ HighlyAbundant.jl +++
GC.gc()
println("\nTEST: (for n ∈ 1:100 V034885(n) end)")
...
# +++ IntPartitions.jl +++
println("\n-- performance test")
println("\nTEST: for n ∈ 1:50, p ∈ IntegerPartitions(n, byNumPart) p end")
@time for n ∈ 1:50, p ∈ IntegerPartitions(n, byNumPart) p end
println("\nTEST: for n ∈ 1:50, p ∈ IntegerPartitions(n, byMaxPart) p end")
...
println("\nTEST: Lah2(1000)")
@time Lah2(1000)
println("\nTEST: RowSum(LahTriangle(1000))")
@time RowSum(LahTriangle(1000))
println("\nTEST: RowSum(LahTriangle(1000))")
@time RowSum(LahTriangle(1000))
# +++ LandauConstants.jl +++
"V001511" |> println
println("\nTEST: for n ∈ 0:100 V001511(n) end")
@time for n ∈ 0:100 V001511(n) end
"V163590" |> println
println("\nTEST: for n ∈ 0:100 V163590(n) end")
@time for n ∈ 0:100 V163590(n) end
"V327492" |> println
println("\nTEST: for n ∈ 0:100 V327492(n) end")
@time for n ∈ 0:100 V327492(n) end
"V005187" |> println
println("\nTEST: for n ∈ 0:100 V005187(n) end")
@time for n ∈ 0:100 V005187(n) end
"V001790" |> println
println("\nTEST: for n ∈ 0:100 V001790(n) end")
@time for n ∈ 0:100 V001790(n) end
"V277233" |> println
println("\nTEST: for n ∈ 0:100 V277233(n) end")
@time for n ∈ 0:100 V277233(n) end
"V327495" |> println
println("\nTEST: for n ∈ 0:100 V327495(n) end")
@time for n ∈ 0:100 V327495(n) end
# +++ NarayanaCows.jl +++
println("\nTEST: (for cow ∈ NarayanasCows(10000)")
@time (for cow ∈ NarayanasCows(10000)
end)
# +++ NodesAndEdges.jl +++
println("\nTEST: [V000055(n) for n in 0:99]")
@time [V000055(n) for n in 0:99]
# +++ NumberTheory.jl +++
println("\nTEST: [Divisors(n) for n ∈ 1:10000]")
@time [Divisors(n) for n ∈ 1:10000]
println("\nTEST: [Radical(n)  for n ∈ 1:10000]")
...
@time EuclidTree(100)
GC.gc()
println("\nTEST: [SchinzelSierpinskiEncoding(l) for l ∈ EuclidTree(100)]")
...
GC.gc()
println("\nTEST: (for row ∈ T132393(500)")
@time (for row ∈ T132393(500)
end)
GC.gc()
println("\nTEST: (for row ∈ T048993(500)")
@time (for row ∈ T048993(500)
end)
GC.gc()
println("\nTEST: (for row ∈ T271703(500)")
