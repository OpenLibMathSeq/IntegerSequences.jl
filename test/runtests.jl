...
# 140b9c82-429f-11ea-0056-1d6cea6f3dea
...
tstidx = joinpath(tstdir, "TESTINDEX.jl")
include(tstidx)
# *** Abundant.jl *********
...
@testset "BernoulliNum" begin
    ...
    @test BernoulliInt(
        3,
        30,
    ) == fmpz(-7708110416280010548302670)
    @test BernoulliInt(
        4,
        40,
    ) == fmpz(-44494882577309421077208834962882560)
    ...
    for m ∈ 1:20
        @test BernoulliInt(m, 200) == BernoulliIntList(
            m,
            200 + 1,
        )[end]
    end
    for n ∈ 0:20
        @test denominator(Rational(Bernoulli(2 *
                                             n))) == ClausenNumber(n)
    end
...
@testset "BoolOps" begin
    ...
    @test BinDigits(12, 17) == (
        [0, 0, 1, 1, 0],
        [1, 0, 0, 0, 1],
    )
    @test [V163617(n) for n ∈ 0:8] == [
        0,
        3,
        6,
        7,
...
@testset "IntegerPartitions" begin
    ...
    @test collect(IntegerPartitions(5)) == reverse(Any[
        [1, 1, 1, 1, 1],
        [2, 1, 1, 1],
        [2, 2, 1],
        [3, 1, 1],
    ...
    for n ∈ 0:7
        ...
        @test length(collect(IntegerPartitions(
            n,
            byMaxPart,
        ))) == pn
        @test length(collect(IntegerPartitions(
...
@testset "SetPartMType" begin end
@testset "SetPartByShape" begin
    @test ShapePartitions(3, 4) == [
        0,
        1,
        220,
        462,
...
@testset "SetPartitions" begin
    ...
    RP = reduce(
        vcat,
        [collect(SetPartitions([1, 2, 3], k)) for k ∈ 1:3],
    )
    @test SP == RP
    for n ∈ 1:7
        s = collect(0:n-1)
...
@testset "SetPartByShape" begin
    @test ShapePartitionsOfTypeM(3, 4) == [
        0,
        1,
        220,
        462,
